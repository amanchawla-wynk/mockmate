...
export type ViewType = 'traffic' | 'intercept' | 'endpoints' | 'states' | 'files';
...
interface ProjectListProps {
  ...
  onAttemptNavigation: (action: () => void, affectsMountedEditor?: boolean) => void;
}
  ...
  return (
    <div className="flex flex-col h-full bg-[#F3F3F3]">
            ...
            {[
              ...
              <li key={view.id}>
                <button
                  ...
                  onClick={() => {
                    if (activeView === view.id) return;
                    onAttemptNavigation(() => onSelectView?.(view.id as ViewType));
                  }}
                  className={`
        ...
        <div>
            ...
            {isDropdownOpen && (
              <div className="absolute top-full left-2 right-2 mt-1 bg-white border border-gray-200 rounded-md shadow-lg z-20 overflow-hidden">
                        ...
                        <button
                          onClick={() => {
                            if (project.id !== activeProjectId) {
                              onAttemptNavigation(() => onSelectProject(project.id));
                            }
                            setIsDropdownOpen(false);
                          }}
                ...
                <div className="border-t border-gray-100 p-1">
                  <button
                    onClick={() => {
                      onAttemptNavigation(onNewProject);
                      setIsDropdownOpen(false);
                    }}
          ...
          {activeProjectId && (
            <ul className="space-y-0.5 px-2 mt-2">
              {[
                {
                  id: 'endpoints' as ViewType,
                  label: 'Endpoints',
                  icon: 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z',
                },
                {
                  id: 'states' as ViewType,
                  label: 'App States',
                ...
                <li key={view.id}>
                  <button
                    ...
                    onClick={() => {
                      if (activeView === view.id) return;
                      onAttemptNavigation(() => onSelectView?.(view.id));
                    }}
                    className={`
                      w-full text-left px-2 py-1.5 flex items-center gap-2 rounded-md transition-colors
                      ${activeView === view.id ? 'bg-blue-500 text-white' : 'text-gray-700 hover:bg-gray-200/60'}
      ...
      <ConfirmDialog
        ...
        title="Delete Project"
        message={projectToDelete ? `Delete Project "${projectToDelete.name}" and all of its Endpoints, Variants, App States, Body Assets, and static files? This action cannot be undone.` : ''}
