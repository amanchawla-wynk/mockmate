...
export function DeviceSetup({ httpPort = 3456, httpsPort = 3457, proxyPort = 8888, localIPs }: DeviceSetupProps) {
  ...
  useEffect(() => {
    const ips = localIPs ?? [];
    const httpURLs = [`http://localhost:${httpPort}`, ...ips.map(ip => `http://${ip}:${httpPort}`)];
    const httpsURLs = [`https://localhost:${httpsPort}`, ...ips.map(ip => `https://${ip}:${httpsPort}`)];

    const preferredHTTP = ips.length > 0 ? `http://${ips[0]}:${httpPort}` : `http://localhost:${httpPort}`;

    setNetworkInfo({
      httpURLs,
      httpsURLs,
      setupURL: `${preferredHTTP}/setup`,
      qrCodeURL: `${preferredHTTP}/setup/qr`,
    });
  }, [httpPort, httpsPort, (localIPs ?? []).join(',')]);
        ...
        <div className="px-6 pb-6 space-y-4">
          {/* Setup Page Link */}
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <button
                     ...
                     className="px-3 py-2 bg-gray-900 text-white rounded hover:bg-black transition-colors text-sm font-medium"
                  >
                    {copied ? 'Copied!' : 'Copy'}
                  </button>
                </div>
            ...
            <div className="mt-4 pt-4 border-t border-gray-200">
              <a
                ...
                className="inline-flex items-center space-x-2 px-4 py-2 bg-gray-900 text-white rounded-lg hover:bg-black transition-all shadow-sm hover:shadow font-medium"
              >
          ...
          <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
            ...
            <ol className="text-xs text-gray-700 space-y-1 list-decimal list-inside">
              ...
              <li>Set device WiFi proxy to your Mac IP on port {proxyPort}</li>
            </ol>
          </div>
