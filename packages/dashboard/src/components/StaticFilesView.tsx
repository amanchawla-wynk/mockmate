...
export function StaticFilesView({ projectId }: StaticFilesViewProps) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const { files: fetched } = await staticFilesApi.list(projectId, controller.signal);
        ...
        <div className="flex items-center gap-4 min-w-0">
          ...
          {files.length > 0 && (
            <div className="text-xs text-gray-400 truncate">
              Served at: <span className="font-mono">/static_files/...</span>
            </div>
          )}
        </div>
...
function FileRow({ file, onDelete, indent }: FileRowProps) {
  const [copied, setCopied] = useState(false);
  const filename = file.path.split('/').pop() ?? file.path;
  const url = `/static_files/${file.path}`;
