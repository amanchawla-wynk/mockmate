...
export function setupPageHTML(context: SetupPageContext): string {
  const primaryIP = context.host;
  const httpsPort = context.ports.https;
  const allIPs = context.localAddresses;
  const proxyPort = context.ports.proxy;
  const httpsURL = `https://${primaryIP}:${httpsPort}`;
  const httpURL = `http://${primaryIP}:${context.ports.http}`;
        ...
        <div class="content">
            ...
            <div class="section">
                ...
                <div class="qr-container">
                    <img src="${httpURL}/setup/qr" alt="Setup QR Code">
            ...
            <div class="section">
                <h2>🌐 Local Server Information</h2>
                <div class="url-box">
                    <div class="label">Setup page:</div>
                    <div class="url">${httpURL}/setup</div>
                </div>
                <div class="url-box">
                    <div class="label">Direct MockMate HTTP / HTTPS listeners:</div>
                    <div class="url">${primaryIP}:${context.ports.http} / ${primaryIP}:${httpsPort}</div>
                </div>
                ${allIPs.length > 1 ? `
            ...
            <div class="section">
                ...
                <p style="margin-bottom: 15px;">
                    Keep your app calling its real backend URL. Configure the device to use MockMate as its HTTP
                    proxy so selected hostnames can be inspected, mocked, passed through, and recorded as Traffic.
                </p>
                <div class="url-box">
                ...
                <ol class="steps">
                    ...
                    <li>
                        <strong>Review the interception allowlist</strong> and explicitly save exact or wildcard hostnames in Project settings. A catch-all <code>*</code> requires confirmation.
                    </li>
                    <li>
                ...
                <div class="warning">
                    <strong>💡 Scope:</strong> Unselected HTTPS remains blind and unrecorded. For selected hosts,
                    unmatched requests and Passthrough Endpoints forward to the incoming original origin. Exact
                    Traffic body capture is off by default.
                </div>
                <div class="warning">
                    <strong>Direct requests:</strong> Direct MockMate requests require a trusted backend Host
